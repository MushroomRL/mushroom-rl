try:
    ...
    def test_prepare_bottom():
        obs = []
        mdp = AirHockeyPrepareBullet(sub_problem="bottom")
        mdp.reset()
        action = np.array([1] * 3)
        ...
        obs_test = np.load("tests/environments/pybullet_envs/air_hockey_prepare_bottom_data.npy")
    ...
    def test_hit_left():
        obs = []
        mdp = AirHockeyHitBullet(init_robot_state="left")
        mdp.reset()
        action = np.array([1] * 3)
        ...
        obs_test = np.load("tests/environments/pybullet_envs/air_hockey_hit_left_data.npy")
