...
def test_logger(tmpdir):
    logger_1 = Logger(seed=1, results_dir=tmpdir)
    logger_2 = Logger(seed=2, results_dir=tmpdir)

    for i in range(3):
        logger_1.log_numpy(a=i, b=2*i+1)
        logger_2.log_numpy(a=2*i+1, b=i)

    a_1 = np.load(str(tmpdir / 'a-1.npy'))
    a_2 = np.load(str(tmpdir / 'a-2.npy'))
    b_1 = np.load(str(tmpdir / 'b-1.npy'))
    b_2 = np.load(str(tmpdir / 'b-2.npy'))
    ...
    logger_1_bis = Logger(append=True, seed=1, results_dir=tmpdir)
