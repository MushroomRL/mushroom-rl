...
class CriticNetwork(nn.Module):
    def __init__(self, input_shape, output_shape, n_features, **kwargs):
        ...
        history_length, n_channels = input_shape[0][0], input_shape[0][1]
        n_input_obs = history_length * n_channels
        ...
        conv_input_shape = (n_input_obs,) + input_shape[0][2:]
    ...
    def forward(self, state, action):
        state = state.reshape(state.shape[0], -1, state.shape[-2], state.shape[-1])
...
class ActorNetwork(nn.Module):
    def __init__(self, input_shape, output_shape, n_features, **kwargs):
        ...
        self._h2 = nn.Conv2d(32, 64, kernel_size=4, stride=2)
        self._h3 = nn.Conv2d(64, 64, kernel_size=3, stride=1)
        conv_input_shape = (n_input,) + input_shape[2:]
        conv_out_size = TorchUtils.compute_flat_output_size(nn.Sequential(self._h1, self._h2, self._h3),
                                                            conv_input_shape)
...
def experiment(n_epochs, n_steps, n_episode_test, render=True, use_cuda=False, seed=None):
    ...
    # Approximator
    actor_input_shape = (history_length,) + mdp.info.observation_space.shape
    ...
    agent = DDPG(mdp.info, policy_class, policy_params, actor_params, actor_optimizer, critic_params, batch_size,
                 initial_replay_size, max_replay_size, tau, history_length=history_length)
