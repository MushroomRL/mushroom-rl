"""
This script shows how to run an episode-based policy search experiment on many copies of the Segway
environment at once, with MultiprocessEnvironment.

The environment is not vectorized itself: MultiprocessEnvironment runs one copy of it per worker process and
batches their observations, so the experiment collects several episodes per step of the core loop.
...
def experiment(alg, params, n_epochs, n_episodes, n_ep_per_fit, n_envs, seed=None):
    np.random.seed(seed)

    # MDP
    mdp = MultiprocessEnvironment(Segway, n_envs=n_envs)
    mdp.seed(seed)

    logger = Logger(alg.name(), results_dir=None)
    logger.log_experiment_info(alg, mdp, n_epochs=n_epochs, n_episodes=n_episodes,
                               n_ep_per_fit=n_ep_per_fit, n_envs=n_envs, **params)
...
if __name__ == '__main__':
    ...
    for alg, params in algs_params:
        experiment(alg, params, n_epochs=20, n_episodes=100, n_ep_per_fit=25, n_envs=15)
