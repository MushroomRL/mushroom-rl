...
from mushroom_rl.utils.isaac_sim import ObservationType, ActuationType
...
class HoneyBadgerWalking(IsaacSim):
    ...
    def __init__(self, num_envs, horizon, domain_randomization=True, camera_pos=(105, 0, 4),
        ...
        additional_data_spec = [
            ...
            ("joint_frictionloss", "", ObservationType.JOINT_FRICTION_STATIC, self._action_spec),
        ...
        super().__init__(usd_path, self._action_spec, observation_spec, num_envs, 0.99, horizon,
        ...
        # update action space
        action_limits = (self._actuation_helper.get_joint_pos_limits() - self._default_joint_angles) / 0.25
        ...
        self.observation_helper.add_obs("actions", self.NUM_JOINTS, self.info.action_space.low,
        ...
        self._gravity = torch.tensor([0., 0., -1.], device=TorchUtils.get_device()).repeat((self.number, 1))
        self._effort_limit = self._actuation_helper.get_joint_max_efforts()

        # domain randomization
        self._np_rng = np.random.default_rng()
        self._current_mixed = False
        self._current_nr_delay_steps = 0
        self._action_history = torch.zeros((self.MAX_NR_DELAY_STEPS + 1, self.number, self.NUM_JOINTS),
                                           device=TorchUtils.get_device())
    ...
    def setup(self, env_indices, obs):
        ...
        if self.domain_randomization:
            ...
            r_factors = torch_rand_float(0.5, 1.5, (len(env_indices), self.NUM_JOINTS), device=TorchUtils.get_device())
        ...
        self.observation_helper.write_data("joint_pos", joint_pos, env_indices)
        self.observation_helper.write_data("joint_vel", joint_vel, env_indices)

        body_vel = torch_rand_float(-0.5, 0.5, (len(env_indices), 6), device=TorchUtils.get_device())
        self.observation_helper.write_data("body_vel", body_vel, env_indices)
        ...
        zero = torch.zeros(self._n_envs, device=TorchUtils.get_device())
    ...
    def _step_finalize(self, env_indices):
        ...
        base_quat = self.observation_helper.read_data("body_rot")
    ...
    def _resample_commands(self, env_ids):
        ...
        self._commands[env_ids, 1] = torch_rand_float(-1., 1., (len(env_ids), 1),
                                                      device=TorchUtils.get_device()).squeeze(1)
        self._commands[env_ids, 3] = torch_rand_float(
            -3.14, 3.14, (len(env_ids), 1), device=TorchUtils.get_device()
        ).squeeze(1)
    ...
    def _get_obs_normilization_vec(self):
        v = torch.ones((self.observation_helper.obs_length), device=TorchUtils.get_device())
    ...
    def _get_noise_scale_vec(self):
        v = torch.zeros((self.observation_helper.obs_length), device=TorchUtils.get_device())
    ...
    def _get_soft_joint_pos_limit(self):
        soft_joint_pos_limits = torch.zeros(self.NUM_JOINTS, 2, device=TorchUtils.get_device(), requires_grad=False)
    ...
    def _create_observation(self, obs):
        ...
        rot = self.observation_helper.read_data("body_rot")
    ...
    def _compute_action(self, action):
        joint_vels = self.observation_helper.read_data("joint_vel")
        joint_positions = self.observation_helper.read_data("joint_pos")
    ...
    def reward(self, obs, action, next_obs, absorbing):
        ...
        reward = r_tracking_lin_vel + r_tracking_ang_vel + r_lin_vel_z + r_ang_vel_xy + r_torques \
            + r_joint_acc + r_feet_air_time + r_collision + r_action_rate + r_joint_pos_limits + r_height
    ...
    def _push_robots(self, env_indices):
        max_vel = 1.
        vels = torch_rand_float(-max_vel, max_vel, (env_indices.shape[0], 2), device=TorchUtils.get_device())
        extended_vels = self.observation_helper.read_data("body_vel", env_indices)
        extended_vels[:, :2] = vels
        self.observation_helper.write_data("body_vel", extended_vels, env_indices)
    ...
    def _init_domain_randomization_parameters(self):
        ...
        self._seen_joint_damping = self.observation_helper.read_data("joint_damping")
        self._seen_joint_stiffness = self.observation_helper.read_data("joint_stiffness")
        self._seen_joint_armature = self.observation_helper.read_data("joint_armature")
        self._seen_joint_frictionloss = self.observation_helper.read_data("joint_frictionloss")

        self._seen_mass = self.observation_helper.read_data("robot_mass")
        self._seen_summed_mass = torch.sum(self._seen_mass, dim=1)
        self._seen_torque_limit = self.observation_helper.read_data("torque_limit")
        ...
        self._seen_foot_scaling = torch.ones((self.number, 4), device=TorchUtils.get_device())
        self.seen_trunk_com = self.observation_helper.read_data("trunk_com")

        self.observation_helper.write_data("max_joint_vel", self._seen_joint_max_vel, reapply_after_reset=True)

        self._seen_p_gain = torch.full((self.number, self.NUM_JOINTS), 20., device=TorchUtils.get_device())
        self._seen_d_gain = torch.full((self.number, self.NUM_JOINTS), 0.5, device=TorchUtils.get_device())
        self._seen_scaling_factor = torch.full((self.number, self.NUM_JOINTS), 0.25, device=TorchUtils.get_device())

        self._unseen_p_gain = torch.full((self.number, self.NUM_JOINTS), 20., device=TorchUtils.get_device())
        self._unseen_d_gain = torch.full((self.number, self.NUM_JOINTS), 0.5, device=TorchUtils.get_device())

        self._default_trunk_mass = self.observation_helper.read_data("trunk_mass")[0].clone().detach()
        self._default_trunk_inertia = self.observation_helper.read_data("trunk_inertia")[0].clone().detach()
        self._default_trunk_com = self.observation_helper.read_data("trunk_com")[0].clone().detach()
        ...
        self._default_joint_range = self.observation_helper.read_data("joint_range")[0].clone().detach()
        ...
        self._nf_trunk_mass = torch.ones((self.number, 1), device=TorchUtils.get_device())
        self._nf_trunk_com = torch.ones((self.number, 1), device=TorchUtils.get_device())
        self._nf_foot_size = torch.ones((self.number, 1), device=TorchUtils.get_device())
        self._nf_joint_damping = torch.ones((self.number, 1), device=TorchUtils.get_device())
        self._nf_joint_stiffness = torch.ones((self.number, 1), device=TorchUtils.get_device())
        self._nf_joint_armature = torch.ones((self.number, 1), device=TorchUtils.get_device())
        self._nf_joint_friction = torch.ones((self.number, 1), device=TorchUtils.get_device())

        self._nf_p_gain = torch.ones((self.number, 1), device=TorchUtils.get_device())
        self._nf_d_gain = torch.ones((self.number, 1), device=TorchUtils.get_device())
        self._nf_motor_strength = torch.ones((self.number, 1), device=TorchUtils.get_device())
        self._joint_position_offset = torch.zeros((self.number, self.NUM_JOINTS), device=TorchUtils.get_device())
    ...
    def sample_unseen_noise_factors(
        ...
        self._joint_position_offset[env_indices] = torch_rand_float(
            -position_offset, position_offset, (n_envs, self.NUM_JOINTS), TorchUtils.get_device()
        )
    ...
    def sample_seen_parameters(
        ...
        self.observation_helper.write_data("trunk_mass", actual_trunk_mass, env_indices, True)
        actual_trunk_inertia = self._default_trunk_inertia + (actual_trunk_mass / self._default_trunk_mass)
        self.observation_helper.write_data("trunk_inertia", actual_trunk_inertia.unsqueeze(1), env_indices, True)
        ...
        self.observation_helper.write_data("trunk_com", actual_trunk_com.unsqueeze(1), env_indices, True)
        ...
        for i, name in enumerate(["FL_foot_scale", "FR_foot_scale", "RL_foot_scale", "RR_foot_scale"]):
            ...
            self.observation_helper.write_data(name, scale, env_indices, True)

        # joint nominal position
        self._seen_joint_nominal_pos[env_indices] = self._default_joint_nominal_pos \
            + torch_rand_float(
                add_joint_nominal_position_min, add_joint_nominal_position_max, (n_envs, self.NUM_JOINTS),
                TorchUtils.get_device()
            )
        # self.observation_helper.write_data("joint_default_pos", self._seen_joint_nominal_pos[env_indices],
        #                                    env_indices)

        # joint torque limit
        self._seen_torque_limit[env_indices] = self._default_torque_limit \
            * (1 + torch_rand_float(
                -torque_limit_factor, torque_limit_factor, (n_envs, self.NUM_JOINTS), TorchUtils.get_device()
            ))
        self.observation_helper.write_data("torque_limit", self._seen_torque_limit[env_indices], env_indices, True)

        # joint max velocity
        self._seen_joint_max_vel[env_indices] = self._default_joint_max_vel \
            * (1 + torch_rand_float(
                -joint_velocity_factor, joint_velocity_factor, (n_envs, self.NUM_JOINTS), TorchUtils.get_device()
            ))
        self.observation_helper.write_data("max_joint_vel", self._seen_joint_max_vel[env_indices], env_indices, True)
        ...
        self._seen_joint_damping[not_stay_at_default_idx] = torch_rand_float(
            joint_damping_min, joint_damping_max, (num_envs_not_default, self.NUM_JOINTS), TorchUtils.get_device()
        )
        self._seen_joint_stiffness[not_stay_at_default_idx] = torch_rand_float(
            joint_stiffness_min, joint_stiffness_max, (num_envs_not_default, self.NUM_JOINTS), TorchUtils.get_device()
        )
        self._seen_joint_armature[not_stay_at_default_idx] = torch_rand_float(
            joint_armature_min, joint_armature_max, (num_envs_not_default, self.NUM_JOINTS), TorchUtils.get_device()
        )
        self._seen_joint_frictionloss[not_stay_at_default_idx] = torch_rand_float(
            joint_friction_loss_min, joint_friction_loss_max, (num_envs_not_default, self.NUM_JOINTS),
            TorchUtils.get_device()
        )

        # chceck if damping is difference in scale
        self.observation_helper.write_data(
        ...
        self._seen_p_gain[env_indices] = 20 + torch_rand_float(
            add_p_gain_min, add_p_gain_max, (n_envs, self.NUM_JOINTS), TorchUtils.get_device()
        )
        self._seen_d_gain[env_indices] = 0.5 + torch_rand_float(
            add_d_gain_min, add_d_gain_max, (n_envs, self.NUM_JOINTS), TorchUtils.get_device()
        )
        self._seen_scaling_factor[env_indices] = 0.25 + torch_rand_float(
            add_scaling_factor_min, add_scaling_factor_max, (n_envs, self.NUM_JOINTS), TorchUtils.get_device()
        )
