...
from mushroom_rl.utils.isaac_sim import ActuationType
...
class QuadrupedIsaac(IsaacSim):
    ...
    terms, the observation of gravity/commands/actions, the joint-position-relative action space, and the
    domain-randomization robot push. A concrete quadruped supplies its own USD asset, controlled joints,
    default pose and collision groups, and is responsible for ``is_absorbing``, ``_compute_torque`` and the
    collision-dependent reward terms, since those genuinely differ between robots.

    """
    def __init__(self, usd_path, action_spec, default_joint_angles, observation_spec, additional_data_spec,
        ...
        Args:
            ...
            domain_randomization (bool): Whether physics-material randomization and the domain-randomized
                robot push are enabled.
    ...
    def _modify_mdp_info(self, mdp_info):
        ...
        self._register_domain_randomization_observations()
    ...
    def _register_domain_randomization_observations(self):
        """
        Hook allowing a subclass to register the observations exposing its domain-randomized parameters to the
        agent, once the observations common to every quadruped are already registered. Registers nothing by
        default.
    ...
    def _sample_setup_joint_pos(self, env_indices):
        """
        Samples the joint configuration a reset environment starts in. Scales the nominal pose randomly by
        default.
    ...
    def _push_domain_randomization(self, env_indices):
        """
        Randomly pushes the robot by giving it a random horizontal velocity, once it has been alive for a
        while. Can be overridden to add further domain randomization happening at every step.
    ...
    def _add_domain_randomization_observations(self, obs):
        """
        Hook allowing a subclass to fill in the domain-randomized observations it registered in
        ``_register_domain_randomization_observations``. Leaves obs untouched by default.
    ...
    def _apply_action_delay(self, action):
        """
        Hook allowing a subclass to delay the action, simulating actuation latency. Returns the action
    ...
    def _compute_torque(self, action, joint_vels, joint_pos):
        """
        Converts the (possibly transformed) action and the current joint state into the torque to apply.
