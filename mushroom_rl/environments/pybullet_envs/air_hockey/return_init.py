...
class AirHockeyReturn(AirHockeySingle):
    ...
    def __init__(self, gamma=0.99, horizon=120, env_noise=False, obs_noise=False, obs_delay=False, torque_control=True,
        ...
        self.spawn_range = np.array([[-0.92, -0.3], [-0.47, 0.47]])
    ...
    def setup(self, state):
        ...
        self.spawn_pos = np.random.rand(2) * (self.spawn_range[:, 1] - self.spawn_range[:, 0]) + self.spawn_range[:, 0]
    ...
    def reward(self, state, action, next_state, absorbing):
        ...
        # print(np.array(robot_conf) - self.desired_conf)
        # print(np.array(robot_conf), self.desired_conf)
