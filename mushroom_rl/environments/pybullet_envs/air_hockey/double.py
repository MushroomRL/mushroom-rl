...
class AirHockeyDoubleBullet(AirHockeyBaseBullet):
    """
    ...
    def _create_observation(self, state):
        ...
        for i in range(2):
            ...
            robot_pos[0] = self.get_sim_state(state, self.agents[i]['name'] + "/joint_1", 
                                              PyBulletObservationType.JOINT_POS).item()
            robot_pos[1] = self.get_sim_state(state, self.agents[i]['name'] + "/joint_2", 
                                              PyBulletObservationType.JOINT_POS).item()
            robot_pos[2] = self.get_sim_state(state, self.agents[i]['name'] + "/joint_3", 
                                              PyBulletObservationType.JOINT_POS).item()
