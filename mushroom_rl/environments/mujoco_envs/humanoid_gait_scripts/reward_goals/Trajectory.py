...
class Trajectory(object):
    ...
    def __init__(self, traj_path, traj_dt=0.01, control_dt=0.01,
        ...
        # if there are split points of a repeating trajectory,
        # these can be used to select steps individually
        # (for example walking gait steps)
        ...
        # -change trajectory sample freq to match simulation control dt
        #   (needed so state steps in simulation and trajectory match)
        # -option to speed up or slow down the simulation velocity
        #   (necessary to change split points and joint velocity accordingly)
        self.traj_dt = traj_dt
        self.control_dt = control_dt
        self.traj_speed_multiplier = traj_speed_mult

        if self.traj_dt != control_dt or traj_speed_mult != 1.0:
            # factor to re-sample the trajectory data points
    ...
    def _get_traj_gait_sub_steps(self, initial_walking_step, number_of_walking_steps=1):
        '''
        Gets individual steps from the complete trajectory based on the split points
        '''
...
class HumanoidTrajectory(Trajectory):
    ...
    def _get_traj_gait_sub_steps(self, initial_walking_step, number_of_walking_steps=1):
        """
        Gets individual steps from the complete trajectory based on the
            split points and the humanoid torso velocity profile along the sub trajectory.
        """
        start_sim_step = self.split_points[initial_walking_step]
        end_sim_step = self.split_points[initial_walking_step + number_of_walking_steps]
    ...
    def _smooth_vel_profile(self, vel, use_simple_mean=False, window_size=1001, polyorder=2):
        """
        Low pass filters the torso velocity, to have a mean velocity
            profile to match.
        """
        if use_simple_mean:
    ...
    def get_next_sub_trajectory(self):
        """
        Gets the next trajectory once the current one reaches it's end.
        """
        raise NotImplementedError
...
class CompleteHumanoidTrajectory(HumanoidTrajectory):
    ...
    def reset_trajectory(self, substep_no=None):
        ...
        # makes initial position start at (x=0)
        self.subtraj = self.trajectory.copy()
