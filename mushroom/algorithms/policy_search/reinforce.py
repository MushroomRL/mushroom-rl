...
class REINFORCE(PolicyGradient):
    ...
    def __init__(self, approximator, policy, gamma, params, features):
        ...
        self.sumdlogpi = None
        self.list_sumdilogpi = []
        self.baseline_num = []
        self.baseline_den = []

        super(REINFORCE, self).__init__(approximator, policy, gamma, params, features)

    def _compute_gradient(self, J):
        baseline = np.mean(self.baseline_num)/np.mean(self.baseline_den)
        grad_Jep = []
        for i,Jep in enumerate(J):
            sumdlogpi = self.list_sumdilogpi[i]
    ...
    def _step_update(self, x, u):
        dlogpi = self.policy.diff_log(self.approximator, x, u)
        self.sumdlogpi += dlogpi

    def _episode_end_update(self, Jep):
        self.list_sumdilogpi.append(self.sumdlogpi)
    ...
    def _init_update(self):
        self.sumdlogpi = np.zeros(self.approximator.weights_shape)