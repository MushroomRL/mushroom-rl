...
class Weighted(TDPolicy):
    ...
    This policy is supposed to be used only in TD algorithms that compute the
    probability of sampling each action (e.g. Weighted Q-Learning). The
    reference to the weights is set by the setter method and the update of the
    weights is done inside the algorithm.
    ...
    def set_weights(self, w):
        """
        Args:
            w (np.array): the probability of sampling each action.
