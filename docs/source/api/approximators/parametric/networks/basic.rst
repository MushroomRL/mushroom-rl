Basic networks
==============

The general-purpose fully-connected architectures, used as a default actor, critic or Q-network when the problem
does not call for anything more specific.

.. automodule:: mushroom_rl.approximators.parametric.networks.linear_network

.. automodule:: mushroom_rl.approximators.parametric.networks.feed_forward_network

.. automodule:: mushroom_rl.approximators.parametric.networks.critic_network

.. automodule:: mushroom_rl.approximators.parametric.networks.q_network
